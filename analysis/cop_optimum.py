...
import os
import sys
# Make runnable from any cwd, no PYTHONPATH needed (same pattern as
# sensitivity.py / superheat_subcool_sweep.py): analysis/ is a direct child of
# the repo root, so one dirname() gets there.
...
def pinch_check_grid(T_room_grid, T_amb_grid, bore_mm=None, refrigerant=None, tol_K=0.05):
    ...
    from task2 import cycle   # local import: cycle.py does not import this module
...
_ETH_DIVERGING = LinearSegmentedColormap.from_list(
    "eth_diverging", [config.ETH_QUAL_BLUE, "white", config.ETH_QUAL_RED])
